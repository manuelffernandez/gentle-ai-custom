...
Bind this to the dedicated `gentle-orchestrator` agent only. Do NOT apply it to executor phase agents such as `sdd-apply-mixed` or `sdd-verify-mixed`.
...
- `/sdd-init-mixed` -> initialize SDD context; detects stack, bootstraps persistence
- `/sdd-explore-mixed <topic>` -> investigate an idea; reads codebase, compares approaches; no files created
- `/sdd-apply-mixed [change]` -> implement tasks in batches; checks off items as it goes
- `/sdd-verify-mixed [change]` -> validate implementation against specs; reports CRITICAL / WARNING / SUGGESTION
- `/sdd-archive-mixed [change]` -> close a change and persist final state in the active artifact store
- `/sdd-onboard-mixed` -> guided end-to-end walkthrough of SDD using your real codebase
...
This applies to `/sdd-new`, `/sdd-ff`, `/sdd-continue`, `/sdd-explore-mixed`, `/sdd-apply-mixed`, `/sdd-verify-mixed`, `/sdd-archive-mixed`, and natural-language equivalents such as "use SDD to add dark mode" / "do it with SDD".
...
- `openspec/config.yaml`, existing SDD artifacts, previous `sdd-init-mixed` results, or installed SDD assets do NOT satisfy session preflight.
...
For a new product/code change request that says to use SDD, start at preflight -> init guard -> explore/proposal (`/sdd-new` equivalent). Never launch `sdd-apply-mixed` just because the user asked to implement a feature.

Only launch `sdd-apply-mixed` when all are true:
...
After the SDD Session Preflight is complete and before executing ANY SDD command (`/sdd-new`, `/sdd-ff`, `/sdd-continue`, `/sdd-explore-mixed`, `/sdd-apply-mixed`, `/sdd-verify-mixed`, `/sdd-archive-mixed`), check if `sdd-init-mixed` has been run for this project:

1. Search Engram: `mem_search(query: "sdd-init-mixed/{project}", project: "{project}")`
2. If found -> init was done, proceed normally
3. If NOT found -> run `sdd-init-mixed` FIRST (delegate to `sdd-init-mixed` sub-agent), THEN proceed with the requested command
...
- **`ask-on-risk`** (default): Ask later if `sdd-tasks-mixed` forecasts high risk or >400 changed lines.
...
Cache the delivery strategy for the session. Pass it as `delivery_strategy` to `sdd-tasks-mixed` and `sdd-apply-mixed` prompts.
...
Cache the chain strategy for the session. Pass it as `chain_strategy` to `sdd-tasks-mixed` and `sdd-apply-mixed` prompts alongside `delivery_strategy`. Do not ask again unless the user changes scope.
...
After `sdd-tasks-mixed` completes and before launching `sdd-apply-mixed`, inspect the task result summary for `Review Workload Forecast`.
...
- **`auto-chain`**: Do not ask about splitting. If `chain_strategy` is not yet cached, ask which chain strategy to use. Then pass to `sdd-apply-mixed`: implement only the next autonomous slice using work-unit commits, with clear start, finish, verification, and rollback boundary.
- **`single-pr`**: STOP and require/record maintainer-approved `size:exception` before `sdd-apply-mixed`.
- **`exception-ok`**: Continue, but pass to `sdd-apply-mixed` that this run uses maintainer-approved `size:exception`.

Do this even in Automatic mode. Automatic mode does not override reviewer burnout protection.

When launching `sdd-apply-mixed`, always include the resolved `delivery_strategy`, `chain_strategy`, and any chosen PR boundary/exception in the prompt.
...
| sdd-init-mixed | google-vertex/gemini-3.1-pro-preview | Bootstrap SDD context |
| sdd-explore-mixed | google-vertex/gemini-3.1-pro-preview-customtools | Reads code, structural - not architectural |
| sdd-propose-mixed | google-vertex-anthropic/claude-opus-4-7@default | Architectural decisions |
| sdd-spec-mixed | google-vertex-anthropic/claude-sonnet-4-6@default | Structured writing |
| sdd-design-mixed | google-vertex-anthropic/claude-opus-4-7@default | Architecture decisions |
| sdd-tasks-mixed | google-vertex-anthropic/claude-sonnet-4-6@default | Mechanical breakdown |
| sdd-apply-mixed | openai/gpt-5.3-codex | Implementation |
| sdd-verify-mixed | openai/gpt-5.5 | Validation against spec |
| sdd-archive-mixed | google-vertex/gemini-3.1-flash-lite | Copy and close |
| sdd-onboard-mixed | google-vertex/gemini-3.1-pro-preview | Guided walkthrough |
...
| Phase         | Reads                                                   | Writes           |
| ------------- | ------------------------------------------------------- | ---------------- |
| `sdd-explore-mixed` | nothing                                                 | `explore`        |
| `sdd-propose-mixed` | exploration (optional)                                  | `proposal`       |
| `sdd-spec-mixed`    | proposal (required)                                     | `spec`           |
| `sdd-design-mixed`  | proposal (required)                                     | `design`         |
| `sdd-tasks-mixed`   | spec + design (required)                                | `tasks`          |
| `sdd-apply-mixed`   | tasks + spec + design + `apply-progress` (if it exists) | `apply-progress` |
| `sdd-verify-mixed`  | spec + tasks + `apply-progress`                         | `verify-report`  |
| `sdd-archive-mixed` | all artifacts                                           | `archive-report` |
...
When launching `sdd-apply-mixed` or `sdd-verify-mixed`, the orchestrator MUST:

1. Search for testing capabilities: `mem_search(query: "sdd-init-mixed/{project}", project: "{project}")`
...
When launching `sdd-apply-mixed` for a continuation batch:
...
| Artifact        | Topic Key                          |
| --------------- | ---------------------------------- |
| Project context | `sdd-init-mixed/{project}`               |
