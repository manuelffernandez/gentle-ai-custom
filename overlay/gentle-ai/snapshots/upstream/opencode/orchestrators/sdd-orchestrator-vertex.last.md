...
Bind this to the dedicated `gentle-orchestrator` agent only. Do NOT apply it to executor phase agents such as `sdd-apply-vertex` or `sdd-verify-vertex`.
...
- `/sdd-init-vertex` -> initialize SDD context; detects stack, bootstraps persistence
- `/sdd-explore-vertex <topic>` -> investigate an idea; reads codebase, compares approaches; no files created
- `/sdd-apply-vertex [change]` -> implement tasks in batches; checks off items as it goes
- `/sdd-verify-vertex [change]` -> validate implementation against specs; reports CRITICAL / WARNING / SUGGESTION
- `/sdd-archive-vertex [change]` -> close a change and persist final state in the active artifact store
- `/sdd-onboard-vertex` -> guided end-to-end walkthrough of SDD using your real codebase
...
This applies to `/sdd-new`, `/sdd-ff`, `/sdd-continue`, `/sdd-explore-vertex`, `/sdd-apply-vertex`, `/sdd-verify-vertex`, `/sdd-archive-vertex`, and natural-language equivalents such as "use SDD to add dark mode" / "do it with SDD".
...
- `openspec/config.yaml`, existing SDD artifacts, previous `sdd-init-vertex` results, or installed SDD assets do NOT satisfy session preflight.
...
For a new product/code change request that says to use SDD, start at preflight -> init guard -> explore/proposal (`/sdd-new` equivalent). Never launch `sdd-apply-vertex` just because the user asked to implement a feature.

Only launch `sdd-apply-vertex` when all are true:
...
After the SDD Session Preflight is complete and before executing ANY SDD command (`/sdd-new`, `/sdd-ff`, `/sdd-continue`, `/sdd-explore-vertex`, `/sdd-apply-vertex`, `/sdd-verify-vertex`, `/sdd-archive-vertex`), check if `sdd-init-vertex` has been run for this project:

1. Search Engram: `mem_search(query: "sdd-init-vertex/{project}", project: "{project}")`
2. If found -> init was done, proceed normally
3. If NOT found -> run `sdd-init-vertex` FIRST (delegate to `sdd-init-vertex` sub-agent), THEN proceed with the requested command
...
- **`ask-on-risk`** (default): Ask later if `sdd-tasks-vertex` forecasts high risk or >400 changed lines.
...
Cache the delivery strategy for the session. Pass it as `delivery_strategy` to `sdd-tasks-vertex` and `sdd-apply-vertex` prompts.
...
Cache the chain strategy for the session. Pass it as `chain_strategy` to `sdd-tasks-vertex` and `sdd-apply-vertex` prompts alongside `delivery_strategy`. Do not ask again unless the user changes scope.
...
After `sdd-tasks-vertex` completes and before launching `sdd-apply-vertex`, inspect the task result summary for `Review Workload Forecast`.
...
- **`auto-chain`**: Do not ask about splitting. If `chain_strategy` is not yet cached, ask which chain strategy to use. Then pass to `sdd-apply-vertex`: implement only the next autonomous slice using work-unit commits, with clear start, finish, verification, and rollback boundary.
- **`single-pr`**: STOP and require/record maintainer-approved `size:exception` before `sdd-apply-vertex`.
- **`exception-ok`**: Continue, but pass to `sdd-apply-vertex` that this run uses maintainer-approved `size:exception`.

Do this even in Automatic mode. Automatic mode does not override reviewer burnout protection.

When launching `sdd-apply-vertex`, always include the resolved `delivery_strategy`, `chain_strategy`, and any chosen PR boundary/exception in the prompt.
...
| Phase         | Reads                                                   | Writes           |
| ------------- | ------------------------------------------------------- | ---------------- |
| `sdd-explore-vertex` | nothing                                                 | `explore`        |
| `sdd-propose-vertex` | exploration (optional)                                  | `proposal`       |
| `sdd-spec-vertex`    | proposal (required)                                     | `spec`           |
| `sdd-design-vertex`  | proposal (required)                                     | `design`         |
| `sdd-tasks-vertex`   | spec + design (required)                                | `tasks`          |
| `sdd-apply-vertex`   | tasks + spec + design + `apply-progress` (if it exists) | `apply-progress` |
| `sdd-verify-vertex`  | spec + tasks + `apply-progress`                         | `verify-report`  |
| `sdd-archive-vertex` | all artifacts                                           | `archive-report` |
...
When launching `sdd-apply-vertex` or `sdd-verify-vertex`, the orchestrator MUST:

1. Search for testing capabilities: `mem_search(query: "sdd-init-vertex/{project}", project: "{project}")`
...
When launching `sdd-apply-vertex` for a continuation batch:
...
| Artifact        | Topic Key                          |
| --------------- | ---------------------------------- |
| Project context | `sdd-init-vertex/{project}`               |
