...
- Before any repo mutation, produce a concise decision summary that states what is new upstream, what to adopt, what to discard, the rationale for each, and the recommended upstream refresh path.
...
| Audit is complete and repo/runtime state is still unchanged | Return the decision summary (`new upstream`, `adopt`, `discard`, `why`, refresh recommendation) and STOP for approval before any mutation. |
...
14. After approval, update this repo if needed: owned assets, approved upstream snapshots, policy, docs, state, and logs together. Run `bash sync-gentle-ai-upstream-assets.sh` when the approved upstream copy in `overlay/gentle-ai/assets/upstream/` should move forward.
15. Only after that, run the recommended upstream refresh path: `gentle-ai sync` or full reinstall.
...
- pre-mutation decision summary with `new upstream`, `adopt`, `discard`, `why`, and refresh recommendation
