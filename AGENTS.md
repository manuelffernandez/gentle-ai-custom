...
- `overlay/gentle-ai/policy/maintenance-intent.md` — semantic source of truth for what to preserve, depure, and enforce in repo-owned orchestrator behavior, including the scoped inline delegation overrides for user-requested 4+ file reads and scoped multi-file writes while preserving hard safety/review/incident gates.
